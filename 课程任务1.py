...
def Logistic(W1, W2, data_x, data_y, lr, epoch):
    for n in range(epoch):
        for i in range(4):
            v1 = np.dot(W1, data_x[i].transpose())
            y1 = sigmoid(v1)    # 输出为4*1
            v2 = np.dot(W2, y1)
            y_hat = sigmoid(v2)  # 输出为1*1
            """back propagation"""
            e = data_y[i] - y_hat
            e1 = W2.transpose()*y_hat*(1-y_hat)*e
            y1 = y1.reshape(4, 1)
            delta1 = y1*(1-y1)*e1   # 计算梯度
...
W1, W2 = Logistic(W1, W2, data_x, data_y, 0.9, 10000)
y_list = []
for i in range(4):
    v1 = np.dot(W1, data_x[i].transpose())
    y1 = sigmoid(v1)
    v = np.dot(W2, y1)
    y_hat = sigmoid(v)
    y_hat = classify(y_hat)  # 分类
    y_list.append(y_hat)
print('real: y:', data_y)
print('predict: y:', y_list)
